...
export class TeamService {
  ...
  async transferLeader(
    ...
    await prisma.$transaction([
      prisma.teamMember.update({
        where: { teamId_userId: { teamId: team.id, userId: leaderId } },
        data: { role: 'MEMBER' }
      }),
      prisma.teamMember.update({
  ...
  async leaveTeam(userId: string, guildId: string): Promise<void> {
    ...
    const member = await prisma.teamMember.findFirst({
      where: {
        user: { discordId: userId, guildId },
        team: { guildId }
      }
    });
    if (!member) throw new NotFoundError('소속 팀');

    await this.repo.removeMember(member.teamId, member.userId);

    const count = await this.repo.countMembers(member.teamId);
    if (count === 0) {
      await this.repo.delete(member.teamId);
    }
  }
  ...
  async acceptApplication(leaderId: string, guildId: string, applicantId: string): Promise<void> {
    ...
    await prisma.$transaction([
      prisma.teamApplication.update({
        where: { teamId_userId: { teamId: team.id, userId: applicantId } },
        data: { status: 'ACCEPTED' }
      }),
      prisma.teamMember.create({
        data: { teamId: team.id, userId: applicantId, role: 'MEMBER' }
      })
    ]);
  }
